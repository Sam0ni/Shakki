...
class Alustajatesti6:
    ...
    def alusta_spritet(self, lauta):
        uusi_ratsu = Ratsu(7 * self.ruudun_koko, 7 * self.ruudun_koko, True)
        self.valkoiset_ratsut.add(uusi_ratsu)
        lauta[7][7] = uusi_ratsu

        uusi_ratsu = Ratsu(0 * self.ruudun_koko, 0 * self.ruudun_koko, True)
        self.valkoiset_ratsut.add(uusi_ratsu)
        lauta[0][0] = uusi_ratsu

        uusi_ratsu = Ratsu(0 * self.ruudun_koko, 7 * self.ruudun_koko, False)
        self.mustat_ratsut.add(uusi_ratsu)
        lauta[7][0] = uusi_ratsu

        uusi_ratsu = Ratsu(7 * self.ruudun_koko, 0 * self.ruudun_koko, False)
        self.mustat_ratsut.add(uusi_ratsu)
        lauta[0][7] = uusi_ratsu
        ...
        uusi_sotilas = Sotilas(6 * self.ruudun_koko, 5 * self.ruudun_koko, True)
        self.valkoiset_sotilaat.add(uusi_sotilas)
        lauta[5][6] = uusi_sotilas

        uusi_sotilas = Sotilas(2 * self.ruudun_koko, 1 * self.ruudun_koko, True)
        self.valkoiset_sotilaat.add(uusi_sotilas)
        lauta[1][2] = uusi_sotilas

        uusi_sotilas = Sotilas(1 * self.ruudun_koko, 2 * self.ruudun_koko, True)
        self.valkoiset_sotilaat.add(uusi_sotilas)
        lauta[2][1] = uusi_sotilas

        uusi_sotilas = Sotilas(1 * self.ruudun_koko, 5 * self.ruudun_koko, False)
        self.mustat_sotilaat.add(uusi_sotilas)
        lauta[5][1] = uusi_sotilas

        uusi_sotilas = Sotilas(2 * self.ruudun_koko, 6 * self.ruudun_koko, False)
        self.mustat_sotilaat.add(uusi_sotilas)
